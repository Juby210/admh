...
func main() {
	...
	jadxOptions := admh.GetDefaultJadxOptions()
	var err error
	if apkFileMode {
		err = admh.ExtractAPK(*apkFile, *workDir, out, jadxOptions)
	} else {
		err = admh.DownloadAndExtractAPK(*packageName, *version, *workDir, out, jadxOptions, aptoide.DownloadUrlResolver)
	}
	if err != nil {
		log.Fatal(err)
	}
